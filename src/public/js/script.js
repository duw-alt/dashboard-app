...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Check for saved citizen ID and auto-fill login form
    const savedCitizenId = localStorage.getItem('lastRegisteredCitizenId');
    if (savedCitizenId && document.getElementById('citizenId')) {
        document.getElementById('citizenId').value = savedCitizenId;
        
        // Show registration success message if we have a saved ID
        const successMsg = document.createElement('div');
        successMsg.className = 'success-message';
        successMsg.textContent = getErrorMsg('registration_success');
        
        // Insert success message before the form
        loginForm.parentNode.insertBefore(successMsg, loginForm);
        
        // Clear the saved ID after showing the message
        localStorage.removeItem('lastRegisteredCitizenId');
    }
        ...
        checkResidentBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (response.ok && data.success) {
                    ...
                    residentSuccessMsg.textContent = getErrorMsg('resident_found_success');
                } else {
    ...
    if (registerForm) {
        registerForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                const data = await response.json();
                if (response.ok) {
                    // Save the citizen ID for auto-filling and showing success message
